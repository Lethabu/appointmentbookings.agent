...
const StatCard = ({ title, value, icon, color = 'text-primary' }) => {
    return (<div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-3 rounded-full bg-opacity-20 ${color.replace('text-', 'bg-')}`}>
          {react_1.default.cloneElement(icon, { className: `h-7 w-7 ${color}` })}
        </div>
      </div>
    </div>);
};
