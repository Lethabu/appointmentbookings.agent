...
async function getSalonForOwner(supabase, userId) {
  const { data: salon } = await supabase
    .from('salons')
    .select('id, plan') // Also get plan to check tier
    .eq('owner_id', userId)
    .single()
  return salon
}
...
export async function POST(req) {
  ...
  if (!salon || (salon.plan !== 'pro' && salon.plan !== 'elite')) {
    return new NextResponse('E-commerce is a Pro/Elite feature.', { status: 403 })
  }

  const body = await req.json()
  const { data, error } = await supabase.from('products').insert({ ...body, salon_id: salon.id }).select()
